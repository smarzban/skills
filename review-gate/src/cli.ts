...
import { collect, type MissingPass } from "./collect.js";
...
const parseArgs = (argv: string[]): { positionals: string[]; flags: Record<string, string[]> } => {
...
const parseMissing = (s: string): MissingPass => {
...
async function main() {
  ...
  switch (cmd) {
    ...
    case "collect": {
      ...
      for (const s of flags.scan ?? []) if (s) names.push(s);     // explicit scan file(s), if stored elsewhere
      const seen = new Set<string>();
      const files = names
        .filter((p) => !seen.has(p) && (seen.add(p), true))       // dedup (a scan globbed AND passed via --scan)
        .map((p) => {
          try { return { name: p, json: JSON.parse(readFileSync(p, "utf8")) }; }
          catch (e) { throw new Error(`collect: cannot parse ${p}: ${e instanceof Error ? e.message : String(e)}`); }
        });
      ...
      const missing: MissingPass[] = (flags.missing ?? []).filter(Boolean).map(parseMissing);
