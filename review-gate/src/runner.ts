...
export function parseFindings(text: string): Finding[] | null {
  ...
  try {
    ...
    if (a) { const rows = validateRows(a); if (a.length === 0 || rows.length > 0) return rows; }
  ...
  for (const m of text.matchAll(FENCE_RE)) {
    ...
    if (a) union.push(...validateRows(a));
  }
...
export function parseClaudeResult(stdout: string): { findings: Finding[] | null; isError: boolean; subtype?: string; resultText: string } {
  let env: any;
  try { env = JSON.parse(stdout); } catch { return { findings: null, isError: true, subtype: "unparseable_envelope", resultText: "" }; }
...
export async function spawnWithDeadline(
  ...
  if (r.code === -1) throw new Error(errorTail(r.stderr) || "spawn failed"); // spawn error / signalled
...
export async function runReview(
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      if (attempt === 1 && isTransientSpawnError(msg)) continue;             // retry a flaky spawn ONCE
